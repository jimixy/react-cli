const { semver, loadModule } = require('@vjcli/cli-shared-utils')
      ...
      if (config.foundFiles.vue) {
        api.addSuggestion({
          ...
          handler () {
            ...
            const { launch } = require('@vjcli/cli-shared-utils')
            launch(file)
            return {
              keep: true
            }
...
async function install (api, id) {
  ...
  try {
    const servicePkg = loadModule('@vjcli/cli-service/package.json', context)
    // @vjcli/cli-plugin-router is not compatible with @vjcli/cli-service v3,
    ...
    if (semver.satisfies(servicePkg.version, '3.x')) {
      await invoke.runGenerator(context, {
        id: `core:${id}`,
        apply: loadModule(`@vjcli/cli-service/generator/${id}`, context)
      })
    } else {
