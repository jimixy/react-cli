...
const { chalk, semver } = require('@vjcli/cli-shared-utils')
...
checkNodeVersion(requiredVersion, '@vjcli/cli')
...
program
  ...
  .action((entry, cmd) => {
    loadCommand('serve', '@vjcli/cli-service-global').serve(entry, cleanArgs(cmd))
  })
...
program
  ...
  .action((entry, cmd) => {
    loadCommand('build', '@vjcli/cli-service-global').build(entry, cleanArgs(cmd))
  })
