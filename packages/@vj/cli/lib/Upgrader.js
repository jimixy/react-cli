...
module.exports = class Upgrader {
  ...
  async getUpgradable (includeNext) {
    ...
    // get current deps
    // filter @vjcli/cli-service, @vue/cli-plugin-* & vue-cli-plugin-*
    for (const depType of ['dependencies', 'devDependencies', 'optionalDependencies']) {
      for (const [name, range] of Object.entries(this.pkg[depType] || {})) {
        if (name !== '@vjcli/cli-service' && !isPlugin(name)) {
          continue
        }
        ...
        if (semver.lt(installed, latest)) {
          // always list @vjcli/cli-service as the first one
          // as it's depended by all other plugins
          if (name === '@vjcli/cli-service') {
