...
const { semver, getPluginLink, toShortPluginId, loadModule } = require('@vjcli/cli-shared-utils')
...
class GeneratorAPI {
  ...
  constructor (id, generator, options, rootOptions) {
    ...
    this.pluginsData = generator.plugins
      .filter(({ id }) => id !== `@vjcli/cli-service`)
      .map(({ id }) => ({
  ...
  get cliServiceVersion () {
    ...
    const servicePkg = loadModule(
      '@vjcli/cli-service/package.json',
  ...
  assertCliServiceVersion (range) {
    ...
    throw new Error(
      `Require @vjcli/cli-service "${range}", but was loaded with "${this.cliServiceVersion}".`
    )
  }
  ...
  addConfigTransform (key, options) {
    ...
    if (
      ...
      if (hasReserved) {
        const { warn } = require('@vjcli/cli-shared-utils')
