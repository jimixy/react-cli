...
- 通过 `@vue/cli` + `@vjcli/cli-service-global` 快速开始零配置原型开发。
- 一个运行时依赖 (`@vjcli/cli-service`)，该依赖：
  - 可升级；
...
CLI 服务 (`@vjcli/cli-service`) 是一个开发环境依赖。它是一个 npm 包，局部安装在每个 `@vue/cli` 创建的项目中。
...
如果你熟悉 [create-react-app](https://github.com/facebookincubator/create-react-app) 的话，`@vjcli/cli-service` 实际上大致等价于 `react-scripts`，尽管功能集合不一样。
